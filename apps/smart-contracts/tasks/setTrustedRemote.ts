import * as crossChainHelper from './crossChainHelper';
...
async function main(taskArgs: any, hre: any) {
    ...
    for (const remote of networks) {
      ...
      try {
        ...
        if (currRemote === trustedRemote) {
          console.log(
            `skipping ${local} ${CONFIG.methodName}(${remoteChainId},${trustedRemote}) ✅ `
          );
          successCounter++;
        } else {
          console.log(
            `${local} calling ${CONFIG.methodName}(${remoteChainId},${trustedRemote})`
          );
          ...
          let tx = await (
            await contract
              .connect(wallet)
              [`${CONFIG.methodName}`](remoteChainId, trustedRemote, {
      ...
      if (failureCounter > 0) {
        console.log(
          `successful: ${successCounter}/${totalRuns} - (${Math.round(
            (successCounter / totalRuns) * 100
          )}%) | failures: ${failureCounter}/${totalRuns} - (${Math.round(
            (failureCounter / totalRuns) * 100
          )}%)`
        );
      } else {
        console.log(
