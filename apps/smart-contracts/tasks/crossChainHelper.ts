...
export function generateCalldata(
  ...
  return `${hre.web3.utils
    .keccak256(`${methodName}(${params.join(',')})`)
    .substring(0, 10)}${hre.web3.eth.abi
...
export const getContractFactory = async (hre, contractName) => {
  if (!contractFactories[contractName]) {
    contractFactories[contractName] = await hre.ethers.getContractFactory(
      contractName
    );
  }
...
export const getContractAtAddress = async (
  hre,
  network,
  contractName,
  contractAddress
...
export async function setTrustedRemote(
  ...
  const args = [remoteChainId, setRemote];
